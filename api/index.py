...
# --- Data Fetching Function (Adapted from previous script) ---
    ...
    try:
        ...
        while since < end_timestamp_ms:
            ...
            try:
                ...
                if last_timestamp >= end_timestamp_ms: app.logger.info("Reached fetch end point."); break # Optimization
...
def index():
        ...
        if df is None:
            ...
            try:
                ...
                if download_format == 'excel':
                    ...
                    # No need to seek(0) when using ExcelWriter context manager apparently
                    output_buffer.seek(0) # Still good practice just in case
    ...
    # List of common exchanges for dropdown (can be expanded)
