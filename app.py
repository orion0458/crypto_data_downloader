...
# Pass these paths to the Flask constructor
app = Flask(__name__,
            template_folder=template_dir,
            static_folder=static_dir)

# Required for flashing messages
app.secret_key = b'_5#y2L"F4Q8z\n\xec]/' # CHANGE THIS to a long random byte string
    ...
    try:
        ...
        while since < end_timestamp_ms:
            ...
            try:
                ...
                if last_timestamp >= end_timestamp_ms: app.logger.info("Reached fetch end point."); break
...
def index():
        ...
        if df is None:
            ...
            try:
                ...
                if download_format == 'excel':
                    ...
                    # Use ExcelWriter context manager for cleaner handling
                    with pd.ExcelWriter(output_buffer, engine='openpyxl') as writer:
                         df.to_excel(writer, sheet_name=sheet_name, index=False)
                    # output_buffer automatically contains the data after the 'with' block
                    # Seek might still be needed depending on how send_file reads it
    ...
    # Pass available exchanges to the template
    return render_template('index.html', exchanges=common_exchanges)


# --- Run the App section (REMOVED FOR VERCEL) ---
# if __name__ == '__main__':
#    # This block is NOT used by Vercel / Gunicorn / Waitress
#    # Only runs if you execute 'python api/index.py' directly for local testing
#    print("Running in local debug mode...")
#    app.run(debug=True, host='0.0.0.0', port=5001) # Use a different port maybe